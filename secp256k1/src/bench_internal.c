...
void bench_field_get_b32(void* arg) {
    int i;
    bench_inv_t *data = (bench_inv_t*)arg;

    for (i = 0; i < 4000000; i++) {
        secp256k1_fe_get_b32(data->data, &data->fe_x);
    }
}

void bench_field_set_b32(void* arg) {
    int i;
    bench_inv_t *data = (bench_inv_t*)arg;

    for (i = 0; i < 4000000; i++) {
        secp256k1_fe_set_b32(&data->fe_x, data->data);
    }
}
...
int main(int argc, char **argv) {
    ...
    if (have_flag(argc, argv, "field") || have_flag(argc, argv, "get_b32")) run_benchmark("field_get_b32", bench_field_get_b32, bench_setup, NULL, &data, 10, 4000000);
    if (have_flag(argc, argv, "field") || have_flag(argc, argv, "set_b32")) run_benchmark("field_set_b32", bench_field_set_b32, bench_setup, NULL, &data, 10, 4000000);
